...
import ProductItem from "../coupon/NewCoupon/ProductSelect/ProductItem";

export default function CreateBuyXGetX() {
  const form = useForm({
    resolver: zodResolver(buyXGetYFreeSchema),
    defaultValues: {
      id: 0,
      name: "",
      description: "",
      banner: "",
      startDate: new Date(),
      gettingType: "percentage",
      discountUnit: "percentage",
      discount: "",
    },
  });
  const discountUnit = form.watch("discountUnit");
  const gettingType = form.watch("gettingType");
  const startDate = form.watch("startDate");
  const endDate = form.watch("endDate");
  const onSubmit = (data: z.infer<typeof buyXGetYFreeSchema>) => {
        ...
        <div className="flex flex-row gap-5">
          <div className="bg-accent flex w-1/2 flex-col gap-4 p-5">
            ...
            <div className="flex flex-col gap-4">
              <RadioGroup defaultValue="amount" className="gap-2">
                <div className="flex flex-row items-center gap-2">
                  <RadioGroupItem value="items">
                    Minimum quantity items
                  </RadioGroupItem>
                  <label className="">Minimum quantity items</label>
                </div>
                <div className="flex flex-row items-center gap-2">
                  <RadioGroupItem value="amount">
                    Minimum product amount
                  </RadioGroupItem>
                  <label className="">Minimum product amount</label>
                </div>
              </RadioGroup>
              <RadioGroup
                    ...
                    <FormItem className="mt-2">
                      <FormControl>
                        <Input
                          ...
                          pattern="[0-9]*"
                          placeholder={
                            gettingType === "percentage" ? "Percentage" : "free"
                          }
                          value={field.value}
                          onChange={(e) => {
                            const numericOnly = e.target.value.replace(
